...
- Vercel only builds previews for PRs that are ready for review (`ignoreCommand` in `vercel.json` runs `scripts/vercel-ignore-build.mjs`); drafts and branches without a PR get none. Marking a PR ready triggers its preview through `.github/workflows/preview-on-ready.yml`, which needs the `VERCEL_TOKEN` repo secret. Production always builds.
